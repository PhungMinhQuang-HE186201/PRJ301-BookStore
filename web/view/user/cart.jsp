<%-- 
    Document   : cart
    Created on : Jun 29, 2024, 5:34:08 PM
    Author     : PMQUANG
--%>
    ...
    <body class="cart">
                            ...
                            <div class="table-content table-responsive mb-15 border-1">
                                <table>
                                    ...
                                    <c:forEach items="${cart.listOrderItems}" var="od">
                                        ...
                                        <tr>
                                            <td class="product-thumbnail"><a href="#"><img src="${pageContext.request.contextPath}/img/product/${p.getImage()}" width="50" height="50" alt="man" /></a></td>
                                            <td class="product-name"><a href="#">${p.name}</a></td>
                                            <td class="product-price"><span class="amount">${p.price}</span></td>
                                            <td class="product-quantity">
                                                <form action="cart?action=changeQuantity" method="POST">
                                                    <input type="hidden" name="id" value="${p.id}"/>
                                                    <c:choose>
                                                        <c:when test="${od.quantity > p.quantity}">
                                                            <input type="number" name="quantity" 
                                                                   value="${p.quantity}" min="1" max="${p.quantity}"
                                                                   onchange="return this.closest('form').submit()"/>
                                                        </c:when>
                                                        <c:otherwise>
                                                            <input type="number" name="quantity" 
                                                                   value="${od.quantity}" min="1" max="${p.quantity}"
                                                                   onchange="return this.closest('form').submit()"/>
                                                        </c:otherwise>
                                                    </c:choose>
                                                </form>
                                            ...
                                            <td class="product-remove">
                                                <form action="cart?action=deleteProduct" method="POST">
                                                    <input type="hidden" name="id" value="${p.id}"/>
                ...
                <div class="row">
                    <div class="col-lg-8 col-md-6 col-12">
                        <div class="buttons-cart mb-30">
                            ...
                            <form action="cart?action=deleteAllProduct" method="POST">
                                <ul>
                                    <li><a href="#" onclick="return this.closest('form').submit()">Remove All</a></li>
                                </ul>
                            </form>
                        </div>
                    </div>
                        ...
                        <div class="cart_totals">
                            ...
                            <div class="wc-proceed-to-checkout">
                                <form action="cart?action=checkOut" method="POST">
                                    <a href="#" onclick="return this.closest('form').submit();">Click to check out</a>
                                </form>
                            </div>
                        </div>
                    </div>
        ...
        <script>
                                        ...
                                        function updateSubTotal() {
                                            ...
                                            document.querySelector('#subtotal').innerHTML = totalCart + "đ";
                                            document.querySelector('#totalCart').innerHTML = totalCart + "đ";
                                        }
        </script>
    </body>
